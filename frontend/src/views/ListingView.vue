...
const price = computed(() => property.value ? formatPrice(property.value.price, property.value.type) : '');
const pricePerM2 = computed(() => property.value ? formatPricePerM2(property.value.price / property.value.m2) : '');

const specRow = computed(() => {
  if (!property.value) return [];
  const {rooms, m2, floor, totalFloors, landM2, propertyKind} = property.value;
  const parts: string[] = [];
  parts.push(`${rooms} rm`);
  parts.push(`${m2} m²`);
    ...
    <template v-if="property">
      ...
      <div class="flex items-start justify-between gap-4 mb-1">
        <div class="min-w-0">
          ...
          <h1 class="mt-1 text-xl leading-snug text-[--color-ink] font-medium">{{ property.title }}</h1>
          <p class="mt-1 text-sm text-[--color-ink-2]">{{ property.address }}</p>
        </div>
        <div class="text-right shrink-0">
          <p class="display-price text-2xl text-[--color-ink] whitespace-nowrap">{{ price }}</p>
          <p class="text-xs text-[--color-ink-2] tabular">{{ pricePerM2 }}</p>
        </div>
      </div>

      <div class="flex items-center flex-wrap gap-x-3 gap-y-1 text-sm text-[--color-ink-2] tabular mt-3">
      <span v-for="(part, i) in specRow" :key="i" class="inline-flex items-center gap-3">
        {{ part }}
        <span v-if="i < specRow.length - 1" class="text-[--color-ink-3]" aria-hidden="true">·</span>
      </span>
      </div>
      ...
      <p class="text-sm text-[--color-ink-2] leading-relaxed">{{ property.description }}</p>
      <hr class="border-none border-t border-[--color-line] my-5"/>
      ...
      <div>
        ...
        <div class="flex items-center gap-3 mb-4">
          ...
          <div>
            <p class="text-sm font-medium text-[--color-ink]">Andris Veinbergs</p>
          </div>
        </div>
        <div class="flex">
          <button
