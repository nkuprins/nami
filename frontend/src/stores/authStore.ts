...
export const useAuthStore = defineStore('auth', () => {
  ...
  async function login(
    ...
    try {
      ...
      logger.warn(
          `[AuthStore] Login rejected: Invalid credentials for ${email} (Status: ${res.status})`
      );
      return 'Invalid email or password.';
    } catch (e) {
      logger.error(
          `[AuthStore] Connection error during login for ${email}:`,
          e
      );
  ...
  async function signup(
    ...
    try {
      ...
      if (res.status === 409) {
        logger.warn(
            `[AuthStore] Registration conflict: ${email} is already taken.`
        );
        return 'This email is already registered.';
      }
      if (res.status === 400 && body.errors?.password) {
        logger.warn(
            `[AuthStore] Registration validation failed for ${email}: ${body.errors.password}`
        );
        return body.errors.password;
      }

      logger.warn(
          `[AuthStore] Registration rejected with code ${res.status} for ${email}`
      );
      return 'Registration failed. Please try again.';
    } catch (e) {
      logger.error(
          `[AuthStore] Connection error during signup for ${email}:`,
          e
      );
      return 'Something went wrong. Please try again.';
    }
  ...
  async function forgotPassword(email: string): Promise<void> {
    ...
    try {
      ...
      logger.warn(
          `[AuthStore] Request failed for forgotPassword (${email}):`,
          e
      );
    }
  }
  ...
  async function resetPassword(
    ...
    try {
      ...
      logger.warn(
          `[AuthStore] Password reset rejected: Token invalid or already consumed. (Status: ${res.status})`
      );
  ...
  async function verifyEmail(token: string): Promise<string | null> {
    ...
    try {
      ...
      logger.warn(
          `[AuthStore] Email verification rejected: Token invalid. (Status: ${res.status})`
      );
      return 'Invalid or already used verification link.';
    } catch (e) {
      logger.error(
          '[AuthStore] Connection error during email verification:',
          e
      );
      return 'Something went wrong. Please try again.';
    }
  ...
  async function resendVerification(email: string): Promise<void> {
    ...
    try {
      ...
      logger.warn(
          `[AuthStore] Request failed for resendVerification (${email}):`,
          e
      );
    }
  }
